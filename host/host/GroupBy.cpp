					...
					int numThread, int numBlock , int _CPU_GPU)
{
	...
	(*h_startPos) = (int*)malloc( sizeof(int)*numGroup ); //HOST_MALLOC( (void *)(*h_startPos), sizeof(int)*numGroup ); //

	cl_readbuffer( *h_startPos, d_startPos, sizeof(int)*numGroup,&index,eventList,&CPU_GPU,&burden,_CPU_GPU);//cl_readbuffer(*h_startPos,d_startPos,sizeof(int)*numGroup,0); //
