...
extern cl_program program_1;              // OpenCL program

extern cl_command_queue CommandQueue[1];// OpenCL command que
...
void kernel_enqueue(int size,int kid,cl_uint work_dim, const size_t *groups, size_t *threads,cl_event *List,int* index,cl_kernel *kernel,int *Flag_CPU_GPU,double * burden,int _CPU_GPU)
{
	...
	ciErr1 = clEnqueueNDRangeKernel(CommandQueue[0], (*kernel), work_dim, NULL, groups, threads, 0, NULL, List);
    checkError(ciErr1, "Error %d in clEnqueueNDRangeKernel");

    ciErr1=clFinish(CommandQueue[0]); //CPU_GPU clFlush
}
...
void cl_getKernel(char* kernelName,cl_kernel *Kernel)
{
    ...
    checkError(ciErr1, "Error in clCreateKernel");
}

void cl_getKernel(char* kernelName,cl_kernel *Kernel, int _CPU_GPU )
{
	cl_int ciErr1;
	if (_CPU_GPU == 0)
	 (*Kernel) = clCreateKernel(program, kernelName, &ciErr1);
	else if (_CPU_GPU == 1)
	 (*Kernel) = clCreateKernel(program_1, kernelName, &ciErr1);
...
void cl_readbuffer(void* to, cl_mem from,size_t size,int CPU_GPU)
{
	...
	checkError(ciErr1, "Error %d in //cl_readbuffer");
...
void cl_readbuffer(void* to, cl_mem from, size_t offset, size_t size,int *index,cl_event *eventList,int *Flag_CPU_GPU,double * burden, int _CPU_GPU)
{
	...
	ciErr1 = clEnqueueReadBuffer(CommandQueue[0], from, CL_TRUE, offset, size, to,0, NULL, &eventList[*index]);
...
{
	cl_int ciErr1;
	ciErr1 = clEnqueueWriteBuffer(CommandQueue[0], to, CL_FALSE, 0, size, from, 0, NULL, NULL);

    checkError(ciErr1, "Error %d in //clEnqueueWriteBuffer easy");
...
void cl_writebuffer(cl_mem to, void* from, size_t size,int *index,cl_event *eventList,int *Flag_CPU_GPU,double * burden, int _CPU_GPU)
{
	...
	ciErr1 = clEnqueueWriteBuffer(CommandQueue[0], to, CL_FALSE, 0, size, from,0, NULL, &eventList[*index]);

    checkError(ciErr1, "Error %d in //clEnqueueWriteBuffer");
...
{	
	...
	ciErr1 = clEnqueueCopyBuffer(CommandQueue[0], src, dest, 0, 0, size, 0, NULL, &eventList[*index]);

    checkError(ciErr1, "Error %d in //cl_copyBuffer");
