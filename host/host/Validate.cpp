...
void validateAggAfterGroupBy_easy( Record * h_Rin, int rLen, int* startPos, int numGroups, int* aggResults, int OPERATOR )
{
	bool pass = true;
	...
	memset(checkResult, 0, sizeof(int)*numGroups);
	int result;
	//int groupIdx = 0;
...
//	int timer=DLL_genTimer(0);
	...
	for( int i = 0; i < rLen; i++ )
	{
		...
		{ 
			...
			//result = 0;
				if( checkResult[index] < S[i].x )
				{
					checkResult[index] = S[i].x;
				}
		}
...
//	endTimer( "cpu aggregration after group by", &timer );
	...
	for( int i = 0; i < numGroups; i++ )
	{
		...
		{
			printf( "Aggregrate test failed! %d: 0x%x(real), 0x%x(sugg)\n", i, checkResult[i], aggResults[i]);
			pass = false;
			break;
		}
	}
